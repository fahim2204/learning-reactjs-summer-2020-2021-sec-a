...
function App() {
 ...
 const [userlist, setUserList] = useState(users);
  ...
  const createuser = (e, user, status) => {
    e.preventDefault()
    if(status === 'add') {
  ...
  return (
    ...
    <Router>
      ...
      <Switch>
          ...
          <Route path='/create'>
              <CreateUser status='add' list={userlist} createCallback={createuser} />
          </Route>
          <Route path='/edit/:id' children={<CreateUser status='edit' list={userlist} createCallback={createuser} />}></Route>
          <Route path='*'>
              404 not found
          </Route>          
      </Switch>
